...
export default function CategoriesSearch({ name, onSelect, categories }: CategoriesListProps) {
    const mapped_categories = categories.filter((category) => {
    ...
    return (
        <div className="bg-white text-sm rounded-xl drop-shadow-xl mt-2 max-h-28 overflow-y-auto">
            { name != '' && !isCategoryCreated && (
                <button className="flex flex-row p-1 w-full" onClick={() => onSelect(name)}>
                    Create: {name}
                </button>
            )}
            { mapped_categories.map((category, index) => (
                <button key={index} className="flex flex-row p-1 w-full" onClick={() => onSelect(category.name)}>
                    {category.name}
                </button>
            ))}
        </div>
