...
equal to 4. Like the majority of our routes (for an authenticated user), all pages have a layout, in our case the `_finances`
...
route("/", "routes/_finance/_index.tsx", () => {
    ...
    route("transactions", "routes/_finance/transactions/_index.tsx", () => {
        ... // any other nested routes
        route("/:id/edit", "routes/_finance/transactions/edit.tsx");
    });
});
```
...
* Use `underscore` if the folder/file is not rendered in the `url`
* Use `index` for the files where the layout of the current folder is defined.
